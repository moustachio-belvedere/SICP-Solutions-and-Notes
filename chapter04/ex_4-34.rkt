...
;; treat as quoted list if pair,
;; assume cons,car,cdr typed in
#|
(begin
...
(define (text-of-quotation exp env)
  (let ((hunk (cadr exp)))
    (if (pair? hunk)
        (eval (qlist-proc hunk) env)
        hunk)))

(install! 'quote (lambda (exp env) (text-of-quotation exp env)))
...
(define (get-first-lazy lst)
  (car (cdadar (cadddr lst))))

(define (user-print object)
  (if (compound-procedure? object)
      ;; if the procedure is `cons` then print as list
      (if (equal? '(m x y) (car (procedure-body object)))
        (begin (display "(")
               (display (get-first-lazy object))
               (display ", ...)"))
        (display
