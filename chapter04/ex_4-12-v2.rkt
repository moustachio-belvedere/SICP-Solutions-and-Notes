...
;; slightly less code duplication than previous
;; solution but could still benefit from more
;; procedure abstractions.

;; However, more procedure abstraction would
;; require careful thought about which variables
;; should be passed as arguments and which
;; should be captured from enclosing scope.

;; As always, function names would also require
;; more thought.
...
(define (mkscan var nullfnc eqfnc)
  (define (scan varvals)
    (cond ((null? varvals)
           (nullfnc scan))
          ((eq? var (caar varvals))
...
(define (mkenclosing-lookup env errmsg var)
  (lambda (scan) (env-loop (enclosing-environment env)
                           scan
                           errmsg
                           var)))

(define (mkset-varval val)
  (lambda (varvals) (set-cdr! (car varvals) val)))

(define (lookup-variable-value var env)
  (let ((errmsg "Unbound variable"))
  (define scan (mkscan var
                 (mkenclosing-lookup env errmsg var)
                 (lambda (varvals) (cdar varvals))))
...
(define (define-variable! var val env)
  (define scan (mkscan var
                 (lambda (scan) (set-first-frame! env
                                   (cons (cons var val)
                                         (first-frame env))))
                 (mkset-varval val)))
