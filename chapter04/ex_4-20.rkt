...
(define (letrec->let expr)
  ...
  (define (lr-vars body) (cadr body))
  (define (lr-proc body) (cddr body))
  (define (add-to-end l0 l1) (append l0 (list l1)))
  (define (traverse-let var-ass var-acc set-acc)
    (if (null? var-ass)
        (append (list var-acc) set-acc (lr-proc expr))
        (traverse-let (cdr var-ass)
                      (add-to-end var-acc (list (caar var-ass)
                                                ''*unassigned*))
                      (add-to-end set-acc (list 'set!
                                                (caar var-ass)
                                                (cadar var-ass))))))

  (append (list 'let) (traverse-let (lr-vars expr) '() '())))

(install! 'letrec
          (lambda (exp env) (eval (letrec->let exp) env)))
